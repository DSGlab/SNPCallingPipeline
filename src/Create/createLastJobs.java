...
public class createLastJobs{
    ...
    public static void run(String INPUT_DIR, String ALIGNER_REF, String REFERENCE, String OUTPUT_DIR, String ISOLATE_ID, boolean isQuakeCorrected, String JOBS_DIR, String LAST_PATH, String SAMTOOLS_PATH, String BCFTOOLS_PATH) throws FileNotFoundException{
        ...
        String ISOLATE_DIR = "LAST-"+COR_ID+ISOLATE_ID+"/";
		PrintWriter out = new PrintWriter(JOBS_DIR+"/"+"last-"+COR_ID+ISOLATE_ID+".sh");

        /*Create Folder where the data will be saved*/
        new File(OUTPUT_DIR+ISOLATE_DIR).mkdirs();
	
		/* Header required by scinet job queue*/
		out.println("#!/bin/bash");

		/* The actual alignment */
		out.println(LAST_PATH+"/src/lastal -Q1 -i1 "+ALIGNER_REF+" "+INPUT_DIR+ISOLATE_ID+"_1"+COR_EXTENSION+".fq > "+OUTPUT_DIR+ISOLATE_DIR+"rOne.maf");
		out.println(LAST_PATH+"/src/lastal -Q1 -i1 "+ALIGNER_REF+" "+INPUT_DIR+ISOLATE_ID+"_2"+COR_EXTENSION+".fq > "+OUTPUT_DIR+ISOLATE_DIR+"rTwo.maf");
		out.println(LAST_PATH+"/src/last-pair-probs "+OUTPUT_DIR+ISOLATE_DIR+"rOne.maf "+OUTPUT_DIR+ISOLATE_DIR+"rTwo.maf > "+OUTPUT_DIR+ISOLATE_DIR+"r.maf" );
		out.println(LAST_PATH+"/scripts/maf-convert -d sam "+OUTPUT_DIR+ISOLATE_DIR+"r.maf > "+OUTPUT_DIR+ISOLATE_DIR+"r.sam");
